...
public class GUIController {
	public static void main(String[] args) {
		...
		for (int i=0; i<20; i++){caravans.add(new Caravan(i));}
		for (int i=0; i<20; i++){tents.add(new Tent(i));}
		for (int i=0; i<20; i++){huts.add(new Hut(i));}
		for (int i=0; i<20; i++){luxuryhuts.add(new LuxuryHut(i));}
		GUI GUI = new GUI();
